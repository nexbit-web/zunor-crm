<!-- src/routes/(app)/masters/+page.svelte -->
<script lang="ts">
  ...
  function runSearch(): void {
    const q = query.trim()
    if (q.length === 0) goto('/masters', { keepFocus: true, noScroll: true })
    else if (q.length >= 3)
      goto(`/masters?q=${encodeURIComponent(q)}`, {
        keepFocus: true,
        noScroll: true,
      })
  }
  ...
  function clearSearch(): void {
    clearTimeout(timer)
    query = ''
    goto('/masters', { keepFocus: true, noScroll: true })
  }
...
<div class="flex flex-col gap-5">
  <div>
    <h1 class="text-foreground text-2xl font-semibold">Майстри</h1>
  ...
  {#if !loading && data.masters.length === 0}
    ...
    >
      {#if data.q}
        За запитом «{data.q}» майстрів не знайдено.
      {:else}
        Майстрів поки немає.
      {/if}
    </div>
              ...
              <Table.Row class="group relative cursor-pointer">
                <Table.Cell class="pl-5">
                  <a
                    ...
                    aria-label={`Відкрити майстра ${master.name ?? master.email}`}
