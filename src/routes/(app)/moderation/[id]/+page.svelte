<!-- src/routes/(app)/moderation/[id]/+page.svelte -->
<script lang="ts">
...
<div class="mx-auto flex max-w-4xl flex-col gap-6">
  ...
  <div class="border-border bg-card flex flex-col gap-5 rounded-2xl border p-6">
    ...
    <dl class="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
      <div class="flex items-center gap-2 text-sm">
        <Mail class="text-muted-foreground size-4" aria-hidden="true" /><span
          class="text-foreground">{m.email}</span
        >
      </div>
      <div class="flex items-center gap-2 text-sm">
        <Phone class="text-muted-foreground size-4" aria-hidden="true" /><span
          class="text-foreground">{m.phone ?? 'Немає'}</span
        >
      </div>
      <div class="flex items-center gap-2 text-sm">
        <MapPin class="text-muted-foreground size-4" aria-hidden="true" /><span
          class="text-foreground">{m.city ?? 'Немає'}</span
        >
      </div>
      <div class="flex items-center gap-2 text-sm">
        <Clock class="text-muted-foreground size-4" aria-hidden="true" /><span
  ...
  <div class="border-border bg-card flex flex-col gap-4 rounded-2xl border p-6">
    <div class="flex flex-wrap items-center justify-between gap-2">
      <h2 class="text-foreground text-lg font-semibold">Рішення модерації</h2>
      <span
        class="inline-flex rounded-full px-2 py-0.5 text-xs font-medium {verif.class}"
      >
        Поточний: {verif.label}
      </span>
    </div>

    {#if mp?.verificationStatus === 'REJECTED' && mp.verificationRejectReason}
      <p class="bg-rose-500/10 text-rose-400 rounded-md px-3 py-2 text-sm">
        Попередня причина відхилення: {mp.verificationRejectReason}
      </p>
    {/if}
      ...
      >
        <div class="flex flex-col gap-1.5">
          ...
          <select
            ...
            name="status"
            bind:value={selected}
            class="border-input bg-background focus-visible:ring-ring h-10 w-full max-w-xs rounded-md border px-3 text-sm outline-none focus-visible:ring-2"
          >
            <option value="VERIFIED">Верифіковано — модерація пройдена</option>
        ...
        {#if selected === 'REJECTED'}
          <div class="flex flex-col gap-1.5">
            <label for="reason" class="text-foreground text-sm font-medium"
              >Причина відхилення</label
            >
            <textarea
              id="reason"
              name="reason"
              ...
              maxlength={500}
              rows={2}
              placeholder="Що саме порушує правила…"
        ...
        <Button
          type="submit"
          disabled={submitting}
          variant={selected === 'REJECTED' ? 'destructive' : 'default'}
          class="w-fit gap-2"
        >
          {#if selected === 'REJECTED'}
    ...
    {:else}
      <p class="text-muted-foreground text-sm">
        У майстра немає профілю — модерувати нічого.
      </p>
    {/if}
  </div>
  ...
  <section class="flex flex-col gap-2">
    ...
    {#if (mp?.categories?.length ?? 0) > 0}
      <div class="flex flex-wrap gap-2">
        {#each mp!.categories as cat (cat)}<Badge variant="secondary"
            >{categoryName(cat)}</Badge
          >{/each}
      </div>
    {:else}
      ...
      <div class="flex flex-col gap-3">
        {#each data.reviews as review (review.id)}
          ...
          >
            ...
            {#if review.comment}<p class="text-muted-foreground text-sm">
                {review.comment}
              </p>{/if}
                ...
                <Table.Cell
                  ><span
                    class="inline-flex rounded-full px-2 py-0.5 text-xs font-medium {orderBadge(
                      order.status,
                    ).class}">{orderBadge(order.status).label}</span
                  ></Table.Cell
                >
